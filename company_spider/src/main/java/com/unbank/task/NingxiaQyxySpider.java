...
public class NingxiaQyxySpider {

	private static String listurl = "http://gsxt.ngsh.gov.cn/ECPS/enterpriseAbnAction_enterpriseList.action?curr_Page=";
	private static Log logger = LogFactory.getLog(NingxiaQyxySpider.class);
	...
	private static String tablePre = "qinghai";

	static {
		// 启动日志
		try {
			PropertyConfigurator.configure(NingxiaQyxySpider.class
		...
		for (int i = 1; i < 87309; i++) {
			...
			for (Element element : listElements) {
				try {
					...
					String nbxh = element.select("#nbxh"+i).attr("value");
					String zch = element.select("#zch"+i).attr("value");
					String enttype = element.select("#enttype"+i).attr("value");
					
					String  jiami="http://gsxt.ngsh.gov.cn/ECPS/dwr/call/plaincall/__System.pageLoaded.dwr";
					headers.clear();
					headers.put("callCount", "1");
					headers.put("windowName", "11");
					headers.put("c0-scriptName", "__System");
					headers.put("c0-methodName", "pageLoaded");
					headers.put("c0-id", "0");
					headers.put("batchId", "0");
					headers.put("page", "%2"+listurl+i);
					headers.put("httpSessionId", "8C51ABB4BD2C2BB029ADF52773D1D102");
					headers.put("scriptSessionId", "");
					String niabaoHtml = Fetchers.post(jiami, headers, charset);
					Document document1 =Jsoup.parse(niabaoHtml,jiami);
					...
					//http://gsxt.ngsh.gov.cn/ECPS/qyxxgsAction_initQyjbqk.action?nbxh=oWzHQ%2BYsLHq5n9jcTdP%2FzuPIaKk4byBJCxa2syD7LE4%3D&qylx=1190;
					//http://gsxt.ngsh.gov.cn/ECPS/qybgxxAction_init.action?nbxh=oWzHQ%2BYsLHq5n9jcTdP%2FzuPIaKk4byBJCxa2syD7LE4%3D&qylx=1190
					//http://gsxt.ngsh.gov.cn/ECPS/qyxxgsAction_initQyjbqk.action?nbxh=tkU%2BLJ6xEab4Vv1x1bis7jGnlcW1CCzI97%2B9PKY1PCo%3D&qylx=9100
					String infoHref1="http://gsxt.ngsh.gov.cn/ECPS/qyxxgsAction_initQyjbqk.action?nbxh=oWzHQ%2BYsLHq5n9jcTdP%2FzuPIaKk4byBJCxa2syD7LE4%3D&qylx=1190";
					String infohtml = fetcher.get(infoHref1, charset);
					Document infodocument = Jsoup.parse(infohtml, infoHref1);
					...
					// 年报
					String nianbaoUrl = infoHref1.replace("businessPublicity",
							"enterprisePublicity");
