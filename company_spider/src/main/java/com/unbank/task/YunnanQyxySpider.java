...
public class YunnanQyxySpider {
	private static String listurl = "http://gsxt.ynaic.gov.cn/notice/search/ent_except_list";
	private static Log logger = LogFactory.getLog(YunnanQyxySpider.class);
	...
	private static String tablePre = "yunnan";

	static {
		// 启动日志
		try {
			PropertyConfigurator.configure(YunnanQyxySpider.class
	...
	/*	sqlMap.put("吊销日期", "diao_time");*/
		...
		for (int i = 1143; i < 13470; i++) {
			...
			params.put("session.token", "ef9a7384-5827-4a4e-9f95-f3e3c1102f17");
