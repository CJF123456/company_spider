...
public class GuanxiQyxySpider {

	private static String listurl = "http://gxqyxygs.gov.cn/exceptionInfoSelect.jspx";
	private static Log logger = LogFactory.getLog(GuanxiQyxySpider.class);
	...
	private static String tablePre = "guangxi";

	static {
		// 启动日志
		try {
			PropertyConfigurator.configure(GuanxiQyxySpider.class
	...
	private static void getToziren(Document infodocument, String ent_id) {
		...
		if (thElements.size() == 0) {
			thElements = touziren.select("th[width=25%]");
		}
