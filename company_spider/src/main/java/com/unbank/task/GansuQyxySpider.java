 ...
 * @resource  已完结
 */
public class GansuQyxySpider {

	private static String listurl = "http://xygs.gsaic.gov.cn/gsxygs/pub!getCommon.do?parm=excplist";
	private static Log logger = LogFactory.getLog(GansuQyxySpider.class);
	...
	private static String tablePre = "gansu";

	static {
		// 启动日志
		try {
			PropertyConfigurator.configure(GansuQyxySpider.class
	...
	public static void main(String[] args) {
		...
		sqlMap.put("注销日期", "diao_time");
		for (int i = 1; i < 5; i++) {
			logger.info("现在采集到"+i+"页");
			Map<String, String> params = new HashMap<String, String>();
			Map<String, String> headers = new HashMap<String, String>(); 
			headers.clear();
			headers.put("pageNo", i + "");
			headers.put("gjz", "");
			String url=listurl+"&queryVal=%20&pageno="+i;
			String html = fetcher.post(url, headers, null, charset);
			...
			for (Element element : listElements) {
				try {
					...
					// 年报
					String num1= infodocument.select("#leftTabs>ul>li:nth-child(2)").attr("onclick").trim();
					String[] sss=num1.split(",");
					String num=sss[1].replaceAll("[^0-9-]", "").trim();
					headers.clear();
					headers.put("pripid", num);
					headers.put("regno", ent_id);
					headers.put("entcate","compan");
					String nianurl="http://xygs.gsaic.gov.cn/gsxygs/pub!viewE.do";
					String niabaoHtml = Fetchers.post(nianurl, headers, charset);
	...
	private static void getExceptInfo(Document infodocument, String ent_id) {
		Map<String, Object> baseMap = new HashMap<String, Object>();
		baseMap.put("ent_id", ent_id);
		Elements baseElements = infodocument.select("#jingyingyichangminglu");
		if (baseElements.size() == 0) {
			return;
		}
		Element baseElement = baseElements.first();
		Elements trElements = baseElement.select("tr#excp_tr1");
		for (Element element : trElements) {
			Elements tdElements = element.select("td");
	...
	private static void getGaoguan(Document infodocument, String ent_id) {
		Map<String, Object> baseMap = new HashMap<String, Object>();
		baseMap.put("ent_id", ent_id);
		Elements baseElements = infodocument.select("#beian");
		if (baseElements.size() == 0) {
			return;
		}
		Element baseElement = baseElements.first();
		Elements trElements = baseElement.select("tr#per_tr1");
		for (Element element : trElements) {
			Elements tdElements = element.select("td");
	...
	private static void getUpdate(Document infodocument, String ent_id) {
		Map<String, Object> baseMap = new HashMap<String, Object>();
		baseMap.put("ent_id", ent_id);
		Elements baseElements = infodocument.select("#changTab");
		if (baseElements.size() ==0) {
			return;
		}
		Element baseElement = baseElements.first();
		Elements trElements = baseElement.select("tr#ch_tr1");
		for (Element element : trElements) {
			Elements tdElements = element.select("td");
	...
	private static void getToziren(Document infodocument, String ent_id) {
		Map<String, Object> baseMap = new HashMap<String, Object>();
		baseMap.put("ent_id", ent_id);
		Elements baseElements = infodocument.select("#touziren");
		if (baseElements.size() == 0) {
			return;
		}
		Element baseElement = baseElements.first();
		Elements trElements = baseElement.select("tr#inv_tr1");
