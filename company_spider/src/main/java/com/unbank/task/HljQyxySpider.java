...
public class HljQyxySpider {

	private static String listurl = "http://gsxt.hljaic.gov.cn/exceptionInfoSelect.jspx";
	private static Log logger = LogFactory.getLog(HljQyxySpider.class);
	...
	private static String tablePre = "hlj";

	static {
		// 启动日志
		try {
			PropertyConfigurator.configure(HljQyxySpider.class
			...
			for (Element element : listElements) {
				try {
					...
					String nianbaoUrl = infoHref.replace("businessPublicity",
							"enterprisePublicity").replaceAll("\\&sourceType=1", "").trim();
